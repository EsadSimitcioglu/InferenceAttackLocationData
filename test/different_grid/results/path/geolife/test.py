...
def create_grid():
    ...
    for i in range(1, 21):
        plt.text(grid_dict[i][0], grid_dict[i][1], str(i), fontsize=12, color='black', weight="bold")
...
def path_metric():
    ...
    with open('../../../../../dataset/geolife/geolife_grid.dat') as f:
    ...
    for method in method_dict:
        ...
        for epsilon in epsilon_list:
            ...
            with open('geolifegrid-' + method + '-' + str(epsilon) + '_path_guess.csv') as f:
                reader = csv.reader(f, delimiter="\t")
                for line in reader:
                    grid_list = line[0].split(" ")
                    grid_list_int = [eval(i) for i in grid_list]
                ...
                for guess, gt in zip(users_grid_guess_list, users_grid_gt_list):
                    for index in range(min(len(gt), len(guess))):
                        ...
                        sum_km += geopy.distance.geodesic(guess_value, gt_value).km

                average_km = sum_km / len(users_grid_guess_list)
                km_list.append(average_km)
