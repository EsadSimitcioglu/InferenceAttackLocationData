...
from LDP.protocols import ESAD_CLIENT, ESAD_AGGREGATOR, SIMPLE_RAPPOR_Client, SIMPLE_RAPPOR_Aggregator

# Parameters for simulation
k = 20  # attribute's domain size (grid size)
epsilon_list = [5, 10, 15]  # number of epsilon for test cases
iter_list = [1, 2, 3, 4, 5]
...
for epsilon in epsilon_list:
    ...
    seed_value = 1
    for user_true_values in users_grid_value_list:
        perturbed_bit_vector_list.append(ESAD_CLIENT(user_true_values, epsilon, seed_value))
        seed_value += 1

    est_freq = ESAD_AGGREGATOR(perturbed_bit_vector_list, 100, k, epsilon)
