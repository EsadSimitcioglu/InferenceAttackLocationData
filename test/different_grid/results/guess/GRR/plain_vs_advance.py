...
from LDP.estimation_different_grid import GRR_estimated_guess, GRR_advance_estimated_guess, GRR_FK_estimated_guess
...
probability_of_guess_grr_advance = list()
probability_of_guess_grr_fk = list()
...
for epsilon in epsilon_list:
    ...
    probability_of_guess_grr_advance.append(GRR_advance_estimated_guess(users_grid_value_list, k, epsilon, 1))
    print("Plain GRR is Ready")
    probability_of_guess_grr_fk.append(GRR_FK_estimated_guess(users_grid_value_list, k, epsilon, 'guess'))
    print("FK GRR is Ready")

print(probability_of_guess_grr_plain)
print(probability_of_guess_grr_advance)
print(probability_of_guess_grr_fk)
plt.ylim(0, 1)
plt.xlim(min(epsilon_list), max(epsilon_list))
plt.plot(epsilon_list, probability_of_guess_grr_plain, label='GRR-Plain', color='red')
plt.plot(epsilon_list, probability_of_guess_grr_advance, label='GRR-Advance', color='blue')
plt.plot(epsilon_list, probability_of_guess_grr_fk, label='GRR-FK', color='yellow')
