from experiment.attack.metrics import (
    experiment_metrics,
)
...
def perturb(protocol, user_trajectory_list):
    ...
    for user_index, user_trajectory in enumerate(user_trajectory_list):
        if protocol.is_hash_used:
            report = [
                protocol.client(user_value, user_index + 1)
        ...
        if protocol.is_bit_vector:
            report = [
                protocol.convert_binary_report_to_decimal(report_string)
...
def guess_plain_user_trajectory(
    ...
    guess_list = [
        hmm_model.guess_user_values(protocol, report) for report in report_list
    ]
    return experiment_metrics(test_type, user_trajectory_list, guess_list, dataset_name)


def guess_plain_user_trajectory_olh(
    protocol, hmm_model, user_trajectory_list, test_type="PA", dataset_name=None
):
    report_list = perturb(protocol, user_trajectory_list)
    guess_list = list()
...
def guess_fk_user_trajectory_olh(
    protocol, hmm_model, user_trajectory_list, test_type="PA", dataset_name=None
):
    report_list = perturb(protocol, user_trajectory_list)
    guess_list = list()
    for user_index, report in enumerate(report_list):
        hmm_model.create_advance_protocol_model(
            protocol, user_trajectory_list, user_index + 1
        )
        guess_list.append(hmm_model.guess_user_values(protocol, report))
    return experiment_metrics(test_type, user_trajectory_list, guess_list, dataset_name)


def guess_advance_user_trajectory(
    protocol,
    hmm_model,
...
def guess_advance_user_trajectory_olh(
    protocol,
    hmm_model,
    user_trajectory_list,
    test_count,
    ...
    for user_index, report in enumerate(report_list):
        ...
        for _ in range(test_count):
            ...
            guess_list = [
                hmm_model.guess_user_values(report) for report in test_report_list
            ]
            hmm_model.create_advance_protocol_model(
                protocol, guess_list, user_index + 1
            )
