...
write_filename = "../geolife/geolife_grid.dat"
...
def create_grids():
    ...
    data = pd.read_csv("geolife.csv",
                       chunksize=10000,
                       usecols=['file', 'date', 'latitude', 'longitude', 'time'])

    out_file = open(write_filename, "w", encoding="utf-8")

    for i, chunk in enumerate(data):
        latitude_list = chunk['latitude'].tolist()
        longitude_list = chunk['longitude'].tolist()
        date_list = chunk['date'].tolist()
        time_list = chunk['time'].tolist()
        ...
        for latiude in latitude_list:
            latiude = latiude[1:len(latiude) - 1]
            latitudes.append(ast.literal_eval(latiude))

        for longitude in longitude_list:
            longitude = longitude[1:len(longitude) - 1]
            longitudes.append(ast.literal_eval(longitude))

        for date in date_list:
            date = date[1:len(date) - 1]
            dates.append(ast.literal_eval(date))

        for time in time_list:
            time = time[1:len(time) - 1]
            times.append(ast.literal_eval(time))

        for user_index in range(len(dates)):

            try:
                first_day = dates[user_index][0]
                first_time = times[user_index][0]
            ...
            for lat, lon, date, time in zip(latitudes[user_index], longitudes[user_index], dates[user_index],
                                            times[user_index]):
