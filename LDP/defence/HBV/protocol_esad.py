...
from LDP.protocols import ESAD_CLIENT
from hidden_markov_model.hidden_markov_model import guess, getAdjacent
from metric.path_distance import ratio_of_guess

from LDP.protocols import GRR_Client, GRR_Aggregator, ESAD_CLIENT, ESAD_AGGREGATOR
...
def perturb(epsilon, k, user_true_value_list):
    ...
    for user_true_values in user_true_value_list:
        bit_vector = ESAD_CLIENT(user_true_values, epsilon, seed_value)
        report_string = ""
...
def experiment(epsilon, k, user_values_list):
    ...
    for index, user_perturbed_report in enumerate(perturbed_reports):
        ...
        model = hmm_model_ESAD(epsilon, k, seed_counter, 'plain')
...
def hmm_model_ESAD(epsilon, k, seed_counter, train_type, user_value_list=None):
